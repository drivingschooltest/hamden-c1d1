...
async function loadQuestions() {
  const res = await fetch("c1_questions.json", { cache: "no-store" });
  if (!res.ok) throw new Error("Could not load c1_questions.json");
...
function showResults() {
  ...
  const phone = localStorage.getItem("quiz_phone") || "81355582 - 03918909";
  document.getElementById("resultUser").textContent = `مدرسة حمدان — ${phone}`;
...
async function init() {
  ...
  document.getElementById("userName").textContent = "مدرسة حمدان";
